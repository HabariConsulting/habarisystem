...
class Auth extends CI_Controller {
	...
	function save_dep(){
		...
		{ 
			 $data= array(
                'dep_name'=> $this->input->post('title'),
                'rate'=> $this->input->post('rate'),
                'description'=> $this->input->post('editor1')
                );
			  $this->load->model('ask_model');
              $this->ask_model->add_dep($data);
	...
	function view_dep($id){
		...
		$this->load->model('ion_auth_model');
			...
			$this->load->model('ask_model');
			$data['department'] = $this->ask_model->view_dep($id);
			$data['page_location']='Departments';	
			$data['content']='view_dep';
	...
	function save_client(){
		...
		$this->load->library('form_validation');
                ...
                $this->form_validation->set_rules('c_number','Contact Number','required');
		...
		{ 
			 $data= array(
			  ...
			  $this->load->model('ask_model');
              $this->ask_model->add_client($data);
...
function delete_client($id){
	...
	function view_client($id){
		...
		$this->load->model('ion_auth_model');
			...
			$this->load->model('ask_model');
			$data['client'] = $this->ask_model->view_client($id);
			$data['page_location']='Clients';	
			$data['content']='view_client';
 			$data['page_title']='Habari Consulting Client';
			$data['page_sub_title']='view';
			$this->load->view('includes/template',$data);
		}
		}
	...
	function view_tasks(){
        ...
        $this->load->model('ask_model');
		$data['task_fetch']= $this->ask_model->task_fetch();
		//$data['count_dep']= $this->ask_model->count_dep();
        $data['page_location']='Task';	
		$data['content']='tasks';
		$data['page_title']='Habari Tasks';
		$data['page_sub_title']='Browse tasks';
		$this->load->view('includes/template',$data);
	...
	function save_task(){
		...
		$this->load->library('form_validation');
                ...
                $this->form_validation->set_rules('editor1','Brief','required');
	...
	function save_job(){
		...
		{ 
			 $data= array(
                ...
                'retainer'=> $this->input->post('optionsRadios1')                                
                );
			  $this->load->model('ask_model');
              $this->ask_model->add_job($data);
...
function view_task($id){
	...
	function create_sheet($user_id){
        ...
        $this->load->model('ask_model');
        $data['task_user']= $this->ask_model->task_user($user_id);       
        $data['page_location']='Timesheet';	
		$data['content']='enter_sheet';
		$data['page_title']='Habari Timesheet';
		$data['page_sub_title']='enter new timesheet';
		$this->load->view('includes/template',$data);
	...
	function save_sheet(){
		...
		$this->load->library('form_validation');
                $this->form_validation->set_rules('selCSI','Task','required');
               $this->form_validation->set_rules('hours','Time Taken','required');                              
        if ($this->form_validation->run() == TRUE)
		{ 
			 $data= array(
                'task_id'=> $this->input->post('selCSI'),
                'hours'=> $this->input->post('hours'),
                'posted_by'=> $this->session->userdata($this->config->item('identity', 'ion_auth'))            
                );
			  $this->load->model('ask_model');
              $this->ask_model->add_sheet($data);
	...
	function view_sheets(){
		...
		$data['page_title']='Habari Timesheets';
		$data['page_sub_title']='Browse timesheets';
		$this->load->view('includes/template',$data);
